...
/**
 * Handles `checkout.session.completed` webhook events for Dodo Payments
 *
 * The `checkout.session.completed` event is triggered when a customer completes a checkout session
 * within the Dodo Payments system.
 *
 * It is triggered for the following scenarios:
 * - Subscription
 * - Donation
 * - Setup intent (adding or updating a payment method)
 *
 * This service delegates the event to the appropriate handler based on the session mode and metadata.
 *
 * The `session` payload structure can be found here: https://dodopayments.com/docs/api/checkout-sessions
 */
module.exports = class DodoCheckoutSessionEventService {
    /**
     * @param {object} deps
     * @param {import('../../DodoAPI')} deps.api
     ...
     * @param {import('dodo').Checkout.Session} session
     */
    ...
    async handleDonationEvent(session) {
        ...
        const donationMessage = donationField?.text?.value || null;
    ...
    async handleSetupEvent(session) {
        ...
        const subscriptions = await member.related('dodoSubscriptions').fetch();
    ...
    async handleSubscriptionEvent(session) {
        ...
        const memberRepository = this.deps.memberRepository;
        let member = await memberRepository.get({email: customer.email});
        ...
        if (!member) {
            ...
            const memberDataWithDodoCustomer = {
                ...memberData,
                dodoCustomer: customer,
                offerId
            };
            member = await memberRepository.create(memberDataWithDodoCustomer);
        } else {
            const payerName = _.get(customer, 'subscriptions.data[0].default_payment_method.billing_details.name');
            const attribution = {
                id: session.metadata?.attribution_id ?? null,
